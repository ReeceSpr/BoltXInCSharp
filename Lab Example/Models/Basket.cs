...
namespace Lab_Example.Models
{
    public class Basket
    {
        ...
        public void UpdateBasket(List<BasketLine> lines)
        {
            ...
            {
                var basketLine = db.BasketLines.FirstOrDefault(b => b.BasketID ==
               BasketID && b.ProductID == line.ProductID);
                if (basketLine != null)
                {
                    if (line.Quantity == 0)
                    {
        ...
        public int GetNumberOfItems()
        {
            ...
            if (GetBasketLines().Count > 0)
            {
                numberOfItems = db.BasketLines.Where(b => b.BasketID ==
               BasketID).Sum(b => b.Quantity);
            }
